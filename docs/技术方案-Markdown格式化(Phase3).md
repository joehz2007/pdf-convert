...
基于 [PRD-PDF切分需求.md](D:\projects01\pdf-convert\docs\PRD-PDF切分需求.md)、[需求.txt](D:\projects01\pdf-convert\docs\需求.txt) 以及 [技术方案-PDF内容提取(Phase2).md](D:\projects01\pdf-convert\docs\技术方案-PDF内容提取(Phase2).md) 中已经冻结的上下游契约，Phase 3 的职责应收敛为：

- 消费 Phase 2 输出的草稿 Markdown、`content.json`、`extract_manifest.json`
- 对每个章节级切片执行**完整性对账**
- 对草稿 Markdown 执行**结构修复与样式标准化**
- 产出可直接进入 Phase 4 合并的**最终版 Markdown**
- 输出校对报告与全局 `format_manifest.json`，保留问题追踪与人工复核标记

本阶段的核心目标不是“重新生成 Markdown”，而是对 Phase 2 的 draft 做**确定性校对与格式化**。为满足“内容完整、没有遗漏”的要求，Phase 3 必须以 `content.json` 为事实基线，而不是仅依赖 Markdown 文本本身做格式美化。
...
- 检查 draft Markdown 是否完整、是否遗漏块级内容
...
Phase 3 不负责重新抽取 PDF 内容，也不负责自由改写文本，而是负责把 Phase 2 的 `draft + content.json` 收敛为**可发布、可追溯、可合并**的标准 Markdown。
...
```text
Phase2 extract_manifest.json + content.json + draft .md
        ↓
...
2. 校验切片目录、`content.json`、draft Markdown、`assets/` 是否存在且相互匹配。
3. 基于 `content.json` 建立**内容覆盖台账**，统计页、块、表格、图片和 overlap 页的期望覆盖范围。
4. 解析 draft Markdown，构建 token 流和块级索引。
5. 通过块对齐算法将 Markdown 片段映射回 `content.json` 的块、表格、图片节点。
6. 输出审计结论：缺失块、重复块、标题层级异常、列表断裂、未闭合代码块、表格损坏、图片路径失效等。
7. 按规则执行修复，生成规范化文档中间表示 `NormalizedDocument`。
8. 由 `renderer.py` 将 `NormalizedDocument` 渲染为原始 Markdown。
...
- 能稳定产出 token 流，便于识别标题、列表、代码块、表格、图片、HTML block
...
- 简单表格统一为 pipe table
- 复杂表格保留 HTML block 或引用 Phase 2 导出的截图
...
- `status=success` 的切片必须同时存在 `content.json` 与 draft `.md`
...
- overlap 页内容是否仍被保留
- 表格、图片是否仍有可访问的 Markdown/HTML/资源引用
...
- 表格维度：`headers`、`rows`、`markdown`、`fallback_html`、`fallback_image`
- 图片维度：`asset_path`、`caption`

再构建 draft Markdown 的**实际覆盖索引**：
...
负责将 draft Markdown 中的文本片段与 `content.json` 的块节点对齐。
...
- 对复杂表格：
  - 优先保留 `fallback_html`
  - 若 HTML 也不稳定，则插入 `fallback_image`
...
- 合并单元格过多
- 单元格内嵌多段富文本
- `fallback_html` 或 `fallback_image` 已被 Phase 2 使用
...
- 抽查最终 Markdown 与 Phase 2 draft 相比是否补齐遗漏
- 抽查复杂表格是否按预期保留 HTML 或图片回退
- 抽查图片路径是否可直接渲染
- 抽查标题层级是否连续、可读
- 抽查 overlap 内容是否仍然存在
...
这样才能同时满足 PRD 中“Markdown 格式化”的目标，以及 `需求.txt` 中“逐章节检查内容完整、格式正确、没有遗漏”的要求，并为 Phase 4 合并提供稳定输入。
